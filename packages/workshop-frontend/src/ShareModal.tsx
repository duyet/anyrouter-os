  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenu.Content
        ...
        align="end"
        sideOffset={6}
        className="themed-floating-shadow-lg !z-[1100] !w-[300px] !min-w-0 rounded-2xl border border-border/70 bg-background p-1 !ring-kumo-line"
      >
                ...
                {sortedShareLinks.map((sk, index) => {
                  ...
                  return (
                    <div key={sk.linkId} className={`group ${index > 0 ? 'border-t border-border/70' : ''} ${landedShareLinkId === sk.linkId ? 'share-row-land' : 'transition-colors duration-150 hover:bg-card/50'} px-3 py-2.5`}>
                      <div className="flex items-center gap-3">
                        <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-gradient-to-br from-muted to-card text-muted-foreground ring-1 ring-inset ring-kumo-line/60">
                          <Link size={14} />
                        </div>
                        <div className="min-w-0 flex-1">
                          {isRenaming ? (
                            <input
                              ...
                              className="block w-full border-0 bg-transparent p-0 text-[13px] leading-[17px] font-medium tracking-[-0.25px] text-foreground outline-none shadow-[inset_0_-1px_0_0_var(--color-kumo-line)] transition-shadow placeholder:font-normal placeholder:text-muted-foreground focus:shadow-[inset_0_-1px_0_0_var(--color-kumo-fill)]"
